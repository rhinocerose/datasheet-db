...
ili9341_color_t const ILI9341_KPMGREEN     = (ili9341_color_t)0x85E0;
    ...
    uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
  ...
  // column address set
  ili9341_spi_write_command_data(lcd, 0x2A, 4, (uint8_t[]){ (x0 >> 8) & 0xFF, x0 & 0xFF,
                            								(x1 >> 8) & 0xFF, x1 & 0xFF });

  // row address set
  ili9341_spi_write_command_data(lcd, 0x2B, 4, (uint8_t[]){ (y0 >> 8) & 0xFF, y0 & 0xFF,
                            								(y1 >> 8) & 0xFF, y1 & 0xFF });

  // write to RAM
  ili9341_spi_write_command(lcd, 0x2C);
    ...
    int16_t x, int16_t y)
{
  ...
  HAL_SPI_Transmit(lcd->spi_hal, (uint8_t *)&color_le, 2U, HAL_MAX_DELAY);
