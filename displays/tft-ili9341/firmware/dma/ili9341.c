...
ili9341_t *ili9341_new(
	...
	GPIO_TypeDef *backlight_port,    uint16_t backlight_pin,
    ili9341_screen_orientation_t orientation,
    ...
    if ( (NULL != reset_port)        && IS_GPIO_PIN(reset_pin)         &&
      ...
      if ( itsSupported != touch_support ||
        ...
        if (NULL != (lcd = malloc(sizeof(ili9341_t)))) {
          ...
          lcd->backlight_port       = backlight_port;
          lcd->backlight_pin        = backlight_pin;
          ...
          HAL_GPIO_WritePin(lcd->backlight_port, lcd->backlight_pin, GPIO_PIN_SET);
        }
      }
    }
  }
